...
public class WhatsAppParser {
  ...
  // ‎[30/08/2023, 7:17:12 PM] Ali Mehdi: ‎<attached: 00000003-PHOTO-2023-08-30-19-17-12.jpg>
  // Some exports include invisible marks (e.g. LRM \u200E) and narrow no‑break space (\u202F) before AM/PM.
  ...
  private static List<ChatMessage> parseExportText(String text, String myName) {
    ...
    for (String raw : lines) {
      // WhatsApp exports can include invisible marks (e.g. \u200E) at the start of some lines.
      String line = raw == null ? "" : stripInvisible(raw).trim();
      ...
      if (hm.matches()) {
        ...
        String firstText = hm.group(8); // message content after "Sender:" on same line (may be empty)
        ...
        // WhatsApp exports usually put the message on the same line after the sender.
        if (firstText != null) {
          String cleaned = stripInvisible(firstText).trim();
          Matcher firstAttach = ATTACH_PATTERN.matcher(cleaned);
          if (firstAttach.matches()) {
            String fileName = firstAttach.group(1).trim();
      ...
      if (current != null) {
        ...
        currentText.append(stripInvisible(raw)); // keep original spacing (minus invisible marks)
      }
    }
  ...
  private static String stripInvisible(String s) {
    if (s == null) return "";
    // Common in WhatsApp exports: BOM and LRM.
